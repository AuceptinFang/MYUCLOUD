...
test('本地默认同源，配置后端后只改写后端路径', () => {
  ...
  for (const path of ['/api/login', '/api/jwgl/session', '/ucloud/upload', '/file/video.mp4?key=a%2Fb', '/office/?furl=x', '/jwgl/jsxsd/']) {
    ...
    assert.equal(resolveBackendUrl(path, origin), `https://u.aucept.in${path}`)
  }
  for (const url of ['https://u.aucept.in/api/login', 'https://example.com/video', '/assets/main.js', '/filename', '//example.com/api']) {
...
test('后端地址规范化，拒绝凭证、非 HTTP 协议及路径', () => {
  ...
  for (const value of ['u.aucept.in', 'javascript:alert(1)', 'https://user:pass@example.com', 'https://example.com/api', 'https://example.com/?secret=x']) {
