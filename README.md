...
GitHub Pages 托管前端，Pages 构建默认连接 `https://u.pub.aucept.in` 后端。也可以拉取源码，在本机运行完整应用。
...
后端地址由构建时的 `VITE_BACKEND_URL` 指定，只填写来源地址，例如 `https://u.pub.aucept.in`，不带路径。Pages 默认使用此地址，本地开发和普通构建默认同源。GitHub Actions 中可以通过仓库变量 **VITE_BACKEND_URL** 覆盖默认地址；修改后需要重新构建发布。

```sh
VITE_BACKEND_URL=https://u.pub.aucept.in npm run build:pages
```
...
将 `u.pub.aucept.in` 的 HTTPS 请求转发到实际 Hono 后端，例如本机 `127.0.0.1:8787`。域名本身不提供后端服务，需要独立部署并保持路径转发。
